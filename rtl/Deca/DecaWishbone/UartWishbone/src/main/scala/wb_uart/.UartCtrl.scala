...
class UartCtrl(g : UartCtrlGenerics = UartCtrlGenerics()) extends Component {
  ...
  def driveFrom(busCtrl : BusSlaveFactory,config : UartCtrlMemoryMappedConfig,baseAddress : Int = 0) = new Area {
    require( busCtrl.busDataWidth == 8 /*|| busCtrl.busDataWidth == 16 || busCtrl.busDataWidth == 32*/)
    ...
    if(config.busCanWriteFrameConfig){
      ...
      busCtrlWrapped.write(uartConfigReg.frame.parity,address = 13,bitOffset = 0)
      busCtrl.busDataWidth match {
        case 8 => busCtrlWrapped.write(uartConfigReg.frame.stop,address = 14,bitOffset = 0)
    ...
    val write = new Area {
      ...
      io.write << stream
      busCtrl.busDataWidth match {
        case 8 => busCtrlWrapped.read(sat255(config.txFifoDepth - fifoOccupancy),address = 6,bitOffset = 0)
      ...
      busCtrlWrapped.read(stream.valid, address = 5, 7)
    }
    ...
    val read = new Area {
      ...
      busCtrl.busDataWidth match {
        case 8 =>
          busCtrlWrapped.readStreamNonBlocking(streamBreaked, address = 1/*, validBitOffset = 7, payloadBitOffset = 0*/)
          busCtrlWrapped.read(sat255(fifoOccupancy),address = 7, 0)
        case 16 =>
    ...
    val interruptCtrl = new Area {
      ...
      busCtrlWrapped.read(writeInt, address = 5, 0)
      busCtrlWrapped.read(readInt , address = 5, 1)
    }
    ...
    val misc = new Area{
      ...
      busCtrlWrapped.read(io.readBreak, 0x11, 0)
      val breakDetected = RegInit(False) setWhen(io.readBreak.rise())
      busCtrlWrapped.read(breakDetected, 0x11, 1)
      busCtrlWrapped.clearOnSet(breakDetected, 0x11, 1)
      val doBreak = RegInit(False)
      busCtrlWrapped.setOnSet(doBreak, 0x11, 2)
      busCtrlWrapped.clearOnSet(doBreak, 0x11, 3)
  ...
  def driveFrom8(busCtrl : BusSlaveFactory,config : UartCtrlMemoryMappedConfig,baseAddress : Int = 0) = {
    require(busCtrl.busDataWidth == 8)
